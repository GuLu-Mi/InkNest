...
test('800x600 at 200 percent in both actual themes keeps notice actions body history and presentation exits reachable', async () => {
  ...
  try {
    ...
    for (const theme of ['light', 'dark'] as const) {
      ...
      await screenshot(app, `save-error-${theme}-200.png`)
      await writeFile(test.info().outputPath(`save-error-${theme}-geometry.json`), JSON.stringify(await page.evaluate(() => Object.fromEntries(['.document-notices', '.error-banner', '.error-banner span', '.error-banner button:first-of-type', '.error-banner button:last-of-type', '.document-toolbar', '.document-stage'].map(selector => { const node = document.querySelector(selector)!; return [selector, { rect: node.getBoundingClientRect().toJSON(), scrollHeight: node.scrollHeight, clientHeight: node.clientHeight }] }))), null, 2))
      await expect(retry).toBeInViewport({ ratio: 1 }); await expect(page.getByRole('button', { name: '另存为…', exact: true })).toBeInViewport({ ratio: 1 })
      expect((await page.locator('.document-stage').boundingBox())!.height).toBeGreaterThanOrEqual(100)
