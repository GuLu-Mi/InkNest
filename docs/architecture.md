...
`DocumentToolbar` 绑定活动文档，展示文件名、保存及另存/关闭下拉，按正文容器可用宽度换行；不向窗口标签栏放置文档保存动作。菜单开关仅为组件展示状态，切换 docId/epoch 或进入受保护操作时收起。保存、另存与关闭仍调用 `useWorkspace` 的既有事务；手动保存等待输入法后复核来源，防止切换标签造成操作转移。底部状态由活动 `TabState` 派生，不把后台保存回执显示成当前文档成功。
