...
`reading-state.ts` 在净化后的源码 DOM 上建立内容块指纹与文字偏移映射，不保存整份正文或 DOM 到后台标签。`PreviewPane` 恢复每来源的折叠与块内滚动，再按可见文字及其视口偏移定位；图表、图片、代码折叠和侧栏重排继续校正同一锚点。加载中的临时滚动不覆盖旧书签，用户滚动、搜索和主动跳转接管定位；原块不存在时才使用像素/比例回退。退出时只保留 renderer 视图快照，取消旧增强与观察器。
...
`DocumentToolbar` 绑定活动文档，展示文件名，仅在可编辑会话的 edit 模式挂载保存及另存/关闭下拉；阅读布局不预留保存列，按正文容器可用宽度换行。菜单开关仅为组件展示状态，离开编辑、切换 docId/epoch 或进入受保护操作时收起。保存、另存与关闭仍调用 `useWorkspace` 的既有事务；手动保存等待输入法后复核来源，防止切换标签造成操作转移。底部状态由活动 `TabState` 派生，不把后台保存回执显示成当前文档成功。

标签使用明确的最大宽度并允许在可用空间内收缩，避免百分比最大宽度参与容器固有尺寸计算时留下长名称空白。“+”紧跟标签滚动区域，剩余空间由其后的窗口拖动区域承担。
